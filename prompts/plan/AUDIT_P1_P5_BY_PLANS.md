...
- замечание про placeholder-статус [main.py](/D:/CRYPTOTEHNOLOG/src/cryptotechnolog/main.py) больше не соответствует текущему коду;
- замечание про version drift между `settings`, `main`, `health` и package metadata больше не является актуальной current issue;
- замечание про import-time singleton settings в старой формулировке частично устарело после перехода на lazy cached `get_settings()`.

### Historical architectural context

- observations про legacy/new risk-path coexistence остаются полезным historical context;
- observations про funding scope vs broader plan expectations остаются historical context, а не current release truth;
- observations про fallback/degradation discipline в старом bootstrap/event-bus контуре нужно читать как historical audit material, а не как текущий phase status.
