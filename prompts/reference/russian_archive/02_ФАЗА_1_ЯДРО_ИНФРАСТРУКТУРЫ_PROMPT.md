...
def configure_logging(log_dir: Path, log_level: str = "INFO") -> None:
    ...
    """
    # РЕАЛИЗУЙТЕ:
    # 1. Создать log_dir если не существует
    # 2. Настроить structlog с процессорами:
    #    - merge_contextvars
    #    - add_log_level
    #    - TimeStamper(fmt="iso", utc=True)
    #    - JSONRenderer() для файла
    # 3. Настроить вывод в файл system.jsonl
...
def get_logger(component: str) -> structlog.BoundLogger:
    ...
    """
    # РЕАЛИЗУЙТЕ:
    # 1. Получить логгер через structlog.get_logger()
    # 2. Привязать component через .bind(component=component)
    # 3. Вернуть bound logger
```
...
- [ ] Redis Manager работает (set/get/delete)
...
Пример:
```
=== rust_components/event_bus/src/event.rs ===
...
🧪 NEXT STEPS:
1. cd rust_components/event_bus && cargo build --release
2. cargo test
...
grep -r "// [A-Z]" rust_components/event_bus/src/
