...
Polymarket path теперь использует CLOB для executable pricing: `Gamma markets -> выбранный market -> clobTokenIds -> CLOB /book -> normalized bid/ask -> matcher`. Если CLOB недоступен или `token_id` отсутствует, это считается diagnostic readiness problem; binary не должен тихо считать edge по Gamma fallback `bid=ask=outcomePrice`.
...
- `warnings` - non-fatal diagnostics, включая `WideExecutableSpread`, если CLOB spread превышает `0.10`,
- `errors`.
