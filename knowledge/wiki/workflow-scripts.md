...
Manual network probes используют короткий bounded retry/backoff для transient HTTP/API failures. Если endpoint не ответил после retries, `status` становится `transient_http_failure`, а не обычным parser/discovery failure. Это помогает отличать сетевую нестабильность от несовместимого payload shape.
...
Polymarket path теперь использует targeted Gamma discovery и CLOB для executable pricing: `Gamma public-search -> broad markets fallback -> выбранный market -> clobTokenIds -> CLOB /book -> normalized bid/ask -> matcher`. Если targeted search не дает candidate, broad `/markets` fallback остается diagnostic path, но не должен скрывать, что запрос слишком широкий. Если CLOB недоступен или `token_id` отсутствует, это считается diagnostic readiness problem; binary не должен тихо считать edge по Gamma fallback `bid=ask=outcomePrice`.
...
- `warnings` - non-fatal diagnostics, включая `WideExecutableSpread`, если CLOB spread превышает `0.10`, и `BroadPolymarketGammaDiscovery`, если top Gamma candidates отвергнуты только по `terms_mismatch`,
- `errors`.
...
Workflow также запускает `scripts/run_live_probe_replay.ps1 -Timestamped`, затем `scripts/summarize_live_probe_replay_reports.ps1` и загружает `artifacts/live_probe_replay_report*.json` как artifact `live-probe-replay-report`, а `artifacts/live_probe_replay_trend_summary.txt` как отдельный artifact `live-probe-replay-trend-summary`. Эти artifacts показывают не только доступность API, но и то, дошёл ли payload до parser, validator, in-memory event journal, matcher и executable-pricing quality counters.
...
Как читать manual network artifacts:

- `network-connectivity-report` отвечает только на вопрос “живы ли публичные endpoints и какова latency”.
- `live-probe-replay-report` показывает, прошли ли live payloads через parser, validator, in-memory journal и matcher.
- `live-probe-replay-trend-summary` быстро показывает matcher readiness, parse/http errors и `edge_quality` totals.
- `phase0-daily-report` собирает всё в короткий обзор; warning по midpoint false positives означает, что midpoint выглядит лучше executable pricing.
...
- сколько reports имеют `transient_http_failure` на probe endpoints;
