...
- `selection_report` с `target_expiry_ts_ms`/`selected_expiry_ts_ms` и UTC `target_expiry_date`/`selected_expiry_date`,
- `probe_reports`,
- `ingestion_report`,
- `replay_summary`,
- `errors`.
...
- отдельную таблицу `Selected Candidates` с выбранной Deribit/Polymarket парой и UTC expiry dates;
- warning, если `strike_distance > 0` или `selected_expiry_ts_ms != target_expiry_ts_ms`, потому что это basis mismatch risk; warning показывает dates, а timestamps остаются машинным JSON-контрактом;
