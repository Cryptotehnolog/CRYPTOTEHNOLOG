...
Workflow также запускает `scripts/run_live_probe_replay.ps1 -Timestamped`, затем `scripts/summarize_live_probe_replay_reports.ps1` и загружает `artifacts/live_probe_replay_report*.json` вместе с `artifacts/live_probe_replay_trend_summary.txt` как artifact `live-probe-replay-report`. Этот artifact показывает не только доступность API, но и то, дошёл ли payload до parser, validator, in-memory event journal, matcher и executable-pricing quality counters.
